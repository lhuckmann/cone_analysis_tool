...
#
def nl(xyz1, xyz2, r_cut, cell, R_ABS_MAT=np.array([])):
    IDX=[]
    NL=[]
    if R_ABS_MAT.any():
...
def read_xyz(filename):
    x, y, z = np.loadtxt(filename, skiprows=2, usecols=(1,2,3)).T
    return x, y, z
...
#   --
#
def angle(xyz1, xyz2, xyz3, cell):
    _, r12=mic(xyz2,xyz1,cell)
    _, r13=mic(xyz3,xyz1,cell)
...
#   --
#
def best_spot(xyz0, xyz2, cell, r, trials):
    sp=[]
    for ii in range(trials):
        sp.append(sphere_point([xyz0[0], xyz0[1], xyz0[2]],r))
    PDM=pdm(np.array(sp), np.array(xyz2), cell)
    dist=np.amax([np.amin(ii) for ii in PDM])
    opt_idx=np.argmax([np.amin(ii) for ii in PDM])
    closest=xyz2[np.argmin(PDM[opt_idx])]
    cone=angle(xyz0, closest, sp[opt_idx],cell)
